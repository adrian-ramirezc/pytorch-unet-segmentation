...
parser.add_argument('-bs'  ,'--batch_size', type=int, default=2, help='Number of imgs per batch')
parser.add_argument('-e'   ,'--epochs',type=int, default=1, helpj='Number of epochs')
parser.add_argument('-nw'  ,'--num_workers', type=int, default=1, help='Number of workers')

parser.add_argument('-imgh'  ,'--image_height', type=int, default=50, help='Image Height')
parser.add_argument('-imgw'  ,'--image_width', type=int, default=50, help='Image Width')
...
def train_fn(loader, model, optimizer, loss_fn, scaler):
    ...
    for batch_idx, (X, y) in enumerate(loop):
        ...
        y = y.to(device = args.device).unsqueeze(1)
        ...
        # update tqdm loop
        loop.set_postifx(loss = loss.item())
...
def main():
    ...
    val_transform = A.Compose(
        [
            A.Resize(height = args.image_height, width = args.image_width),
            A.Normalize( # just dividing by 255
                mean = [0.0, 0.0, 0.0],
                std = [1.0, 1.0, 1.0],
